  ...
  return (
    <div className={`flex flex-col flex-1 min-h-screen pb-24 transition-colors duration-200 ${
      ...
      <main className="flex-1 p-4 max-w-6xl mx-auto w-full space-y-6">
        ...
        {/* Tab Navigation */}
        <div className={`flex items-center gap-2 p-1.5 rounded-2xl mt-4 mb-6 select-none shadow-2xs ${
          isDark ? 'bg-gray-900/50 border border-gray-800/80' : 'bg-gray-100/80 border border-gray-200/80'
        }`}>
          <button
            type="button"
            onClick={() => setActiveTab('active')}
            className={`flex-1 py-3 px-4 rounded-xl text-xs font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 border ${
              activeTab === 'active'
                ? isDark
                  ? 'bg-gray-800 text-white shadow-sm border-gray-700/60'
                  : 'bg-white text-gray-900 shadow-xs border-gray-200/90'
                : 'border-transparent text-gray-500 hover:text-gray-700 dark:hover:text-gray-300'
            }`}
          >
            <span>Active</span>
            <span className={`text-[10px] min-w-[22px] px-2 py-0.5 rounded-full font-mono font-extrabold text-center ${
              activeTab === 'active'
        ...
        {activeTab === 'active' ? (
          ...
          <section className="space-y-7">
            ...
            {/* Mobile Lane Sub-Tabs Selector Container */}
            <div className={`grid grid-cols-2 gap-2.5 md:hidden mb-6 p-2 rounded-2xl border shadow-2xs select-none ${
              isDark ? 'bg-gray-900/40 border-gray-800/80' : 'bg-gray-100/70 border-gray-200/80'
            }`}>
              {[
                { 
                  id: 'awaiting', 
                  ...
                  inactiveClass: isDark ? 'bg-gray-800/80 text-amber-400 border-gray-700 hover:bg-gray-800' : 'bg-white text-amber-800 border-amber-200/90 hover:bg-amber-50/60 shadow-2xs'
                },
                { 
                  id: 'sent', 
                  label: 'Sent', 
                  ...
                  inactiveClass: isDark ? 'bg-gray-800/80 text-blue-400 border-gray-700 hover:bg-gray-800' : 'bg-white text-blue-800 border-blue-200/90 hover:bg-blue-50/60 shadow-2xs'
                },
                { 
                  id: 'approved', 
                  label: 'Approved', 
                  ...
                  inactiveClass: isDark ? 'bg-gray-800/80 text-emerald-400 border-gray-700 hover:bg-gray-800' : 'bg-white text-emerald-800 border-emerald-200/90 hover:bg-emerald-50/60 shadow-2xs'
                },
                { 
                  id: 'declined', 
                  label: 'Declined', 
                  ...
                  inactiveClass: isDark ? 'bg-gray-800/80 text-red-400 border-gray-700 hover:bg-gray-800' : 'bg-white text-red-800 border-red-200/90 hover:bg-red-50/60 shadow-2xs'
                }
              ].map((lane) => (
                <button
                  key={lane.id}
                ...
                >
                  ...
                  <span className={`text-[10px] min-w-[22px] text-center px-1.5 py-0.5 rounded-full font-mono font-extrabold ${
                    activeMobileLane === lane.id ? 'bg-white/20 text-white' : 'bg-gray-150 border border-current/20 text-gray-800'
                  }`}>
                    {lane.count}
                  </span>
                </button>
            ...
            <div className={`space-y-3 ${searchQuery || activeMobileLane === 'awaiting' ? 'block' : 'hidden md:block'}`}>
              <div className={`flex items-center gap-2 border-b pb-2 ${isDark ? 'border-gray-800' : 'border-gray-200'}`}>
